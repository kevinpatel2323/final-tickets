...
const SIREN_PATH = '/Users/kevinpatel/Desktop/Report/police-siren-sound-effect-317645.mp3'
const API_URL = 'https://www.district.in/gw/consumer/events/v1/event/getBySlug/tata-ipl-2025-finals-match-in-ahmedabad-june03?tagSummaryView=true';
...
async function checkSale() {
  try {
    ...
    console.log(`[${new Date().toISOString()}] isLive: ${isLiveCalendar}`);

    if ((isLiveCalendar || isLiveCalendarTime != 0 ) && !alarmTriggered) {
      alarmTriggered = true;
      console.log('🔥 Sale is live! Playing siren...');
      player.play(SIREN_PATH, err => {
        if (err) console.error('Error playing siren:', err);
      });
    }
    ...
    console.log(`[${new Date().toISOString()}] isLive: ${isLiveSales}`);
